...
export function ScheduleRepeated(nameOrOptions: QueueManagerScheduleOptions): AsyncMethodDecorator;
export function ScheduleRepeated(nameOrOptions: string, interval: number, expirationTime?: number): AsyncMethodDecorator;
export function ScheduleRepeated(
  nameOrOptions: QueueManagerScheduleOptions | string,
  interval?: number,
  expirationTime = 6e5,
): AsyncMethodDecorator {
  const options: QueueManagerScheduleOptions = getOptions(nameOrOptions, interval, expirationTime);
  ...
  function getOptions(
    ...
    const opts = { name: '', interval: 0, expirationTime: 0 };
  ...
  function checkOptions(opts: QueueManagerScheduleOptions): void {
    ...
    if (!+opts.interval || opts.interval < 0) {
      throw new Error('Interval should be a positive integer for ScheduleRepeated decorator');
    }
  }
}
